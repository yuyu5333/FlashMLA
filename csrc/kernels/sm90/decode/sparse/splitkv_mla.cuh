    ...
    if (warpgroup_idx == 0) {
        ...
        for (int batch_idx = sched_meta.begin_req_idx; batch_idx <= sched_meta.end_req_idx; ++batch_idx) {
            ...
            for (int round = 0; round < NUM_TOKENS_PER_THREAD; ++round) {
                if (IS_V32_LIKE || args.start_block_idx < args.num_orig_kv_blocks)
            ...
            auto process_one_block = [&](int block_idx, auto is_extra_block_t, auto is_first_extra_block_t) {
                ...
                for (int round = 0; round < NUM_TOKENS_PER_THREAD; ++round) {
                    ...
                    if constexpr (!IS_EXTRA_BLOCK) {
                        ...
                        if (block_idx+1 != (IS_V32_LIKE ? args.end_block_idx : args.num_orig_kv_blocks))
...
void KernelTemplate<MODEL_TYPE, NUM_HEADS, EXTRA_MODEL_TYPE>::run(const SparseAttnDecodeParams &params) {
    ...
    if constexpr (MODEL_TYPE == ModelType::V4) {
        ...
        // Bytes per token: 656 for V3.2 (512 fp8 + 4 float32 + 64 bfloat16), 528 for V3.2-no-RoPE (512 fp8 + 4 float32)
        KU_ASSERT(params.stride_kv_row == HEAD_DIM_NOPE + NUM_SCALES*(int)sizeof(float) + HEAD_DIM_ROPE*(int)sizeof(bf16));
    }
